...
function RegionPicker({ onSelect }: { onSelect: (code: (typeof REGION_OPTIONS)[number]['code']) => void }) {
  return (
    <motion.div
      ...
      className="bg-white/5 border border-white/10 rounded-3xl p-4 sm:p-6 w-full max-w-2xl mx-auto"
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
        {REGION_OPTIONS.map((opt, idx) => (
          <motion.button
            ...
            className="relative overflow-hidden rounded-2xl bg-white/10 border border-white/20 p-3 sm:p-4 text-left backdrop-blur hover:bg-white/20 transition shadow-lg"
          ...
          >
            ...
            <p className="text-lg sm:text-xl font-bold text-white">{opt.label}</p>
